program MonteCarloSputtering3D_Cr
    ...
    open(unit=11, file='Cr_3d_slice_z10.dat', status='replace', iostat=iostat) ! Ejemplo de slice en z=10
    if (iostat /= 0) then
        print *, 'Error al abrir el archivo Cr_3d_slice_z10.dat'
        stop
    end if
    ...
    print *, 'Simulación completada. Datos en Cr_height.dat and Cr_3d_slice_z10.dat'
...
contains
    ...
    subroutine interact(xpos, ypos, energy, theta, substrate, chemisorbed, height, nx, ny, Lz)
        ...
        sputter_prob = 0.05 * (energy / (E_sput_Cr * eV_to_J))
...
! set title "2D Height Map of Cr on Mg Substrate"
! set xlabel "X"
! set ylabel "Y"
! set zlabel "Height"
! set grid
! plot 'Cr_height.dat' using 1:2:3 with linespoints
! pause -1 "Press Enter to exit"

! 3D Surface Plot
! Save as 'plot_3d.gp' and run with 'gnuplot plot_3d.gp'
! set datafile separator whitespace
! set title "3D Surface of Cr on Mg Substrate"
! set xlabel "X"
! set ylabel "Y"
! set zlabel "Height"
! set grid
! set pm3d map
! splot 'Cr_height.dat' using 1:2:3 with pm3d
! pause -1 "Press Enter to exit"

! 3D Slice Plot (e.g., z=10)
! Save as 'plot_slice.gp' and run with 'gnuplot plot_slice.gp'
! set datafile separator whitespace
! set title "Slice at Z=10 of Cr on Mg Substrate"
! set xlabel "X"
! set ylabel "Y"
! set zlabel "Material"
! set pm3d map
! splot 'Cr_3d_slice_z10.dat' using 1:2:3 with pm3d
! pause -1 "Press Enter to exit"
