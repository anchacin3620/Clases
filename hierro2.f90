...
PROGRAM simulacion_kmc_pulverizacion
      ...
      INTEGER, PARAMETER :: TAM_REJILLA = 300, MAX_PARTICULAS = 1000000
      ...
      SUBROUTINE simular_evento(altura, num_eventos, max_part, tasa, tiempo, energia_act, temp, kb, tam)
          ...
          IF (num_eventos <= max_part) THEN
              ...
              IF (prob_ads > r) THEN
                  ...
                  IF (prob_dif > r) THEN
                      ...
                      SELECT CASE (direccion)
                          CASE (0); nuevo_y = y - 1; IF (nuevo_y < 1) nuevo_y = tam
                          CASE (1); nuevo_y = y + 1; IF (nuevo_y > tam) nuevo_y = 1
                          CASE (2); nuevo_x = x - 1; IF (nuevo_x < 1) nuevo_x = tam
                          CASE (3); nuevo_x = x + 1; IF (nuevo_x > tam) nuevo_x = 1
                      END SELECT
              ...
              ELSE
                  ...
                  IF (prob_pulv > r_pulv .AND. altura(x,y) > 0) altura(x,y) = altura(x,y) - 1.0
              END IF
          END IF
