...
PROGRAM simulacion_montecarlo_pulverizacion_3d_cr
      ...
      DO WHILE (num_eventos < MAX_PARTICULAS)
          CALL simular_evento(material, num_eventos, MAX_PARTICULAS, TASA_EVENTOS, tiempo, ENERGIA_DIFUSION, TEMP_SUBSTRATO, KB, ADHESION, TEMP_PLASMA, TAM_REJILLA, ALTURA_MAX, depositos)
      ...
      SUBROUTINE simular_evento(material, num_eventos, max_part, tasa_eventos, tiempo, energia_difusion, temp_substrato, kb, adhesion, temp_plasma, tam_rejilla, altura_max, depositos)
          ...
          IF (num_eventos <= max_part) THEN
              ...
              ! Calcular z_max dinámicamente antes de cada evento
              z_max = 2
              ...
              IF (prob_adhesion > r) THEN
                  ...
                  material(x,y,z_max+1) = 1  ! Depósito en la siguiente capa
                  ! Difusión solo si hay altura suficiente
                  IF (z_max > 2) THEN
                      prob_difusion = MIN(1.0, EXP(-energia_difusion / (kb * temp_substrato)))
                      CALL RANDOM_NUMBER(r)
                      IF (prob_difusion > r) THEN
                          CALL RANDOM_NUMBER(r)
                          ...
                          END SELECT
                          material(nuevo_x, nuevo_y, z_max+1) = 1
