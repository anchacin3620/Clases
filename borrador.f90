...
CONTAINS
    ...
    SUBROUTINE simular_evento(material, altura_max_local, num_eventos, max_part, tasa_eventos, tiempo, energia_difusion, temp_substrato, kb, adhesion, temp_plasma, tam_rejilla, altura_max, depositos, prob_difusion_const)
        ...
        REAL :: energia, prob_adhesion, r, r_tiempo, temp_real
        REAL, PARAMETER :: MIN_R = 1.0E-10, MAX_ENERGIA = 5.0

        CALL RANDOM_NUMBER(r)
        temp_real = -LOG(MAX(MIN_R, r)) * tasa_eventos
        num_eventos = num_eventos + MAX(1, INT(FLOOR(temp_real)))
        ...
        IF (num_eventos <= max_part) THEN
            ...
            CALL safe_random(r)
            prob_adhesion = adhesion * (1.0 - r * 0.2)  ! Ajuste para más depósitos
            PRINT *, 'Evento en (', x, ',', y, '), z_max = ', z_max, ', prob_adhesion = ', prob_adhesion, ', r = ', r
            IF (prob_adhesion > r .AND. z_max < altura_max) THEN
                depositos = depositos + 1
                material(x,y,z_max+1) = 2
                altura_max_local(x,y) = z_max + 1
                PRINT *, 'Depósito en (', x, ',', y, '), nuevo z_max = ', z_max + 1
                IF (z_max > 2) THEN
                    CALL safe_random(r)
