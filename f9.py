...
def identificar_etapa(datos):
    ...
    if espesor < UMBRALES_ETAPAS['nucleacion'] and cobertura < 20 and densidad_islas > 0.005:
        return 'nucleacion'
...
def load_simulation_data(filename):
    """Cargar datos de altura de la simulación (sin generar gráficas individuales)"""
    try:
...
def generar_graficos_comparativos(todos_datos):
    """Generar gráficos comparativos entre materiales"""
    if not todos_datos:
...
def generar_graficos_promedio(todos_datos):
    ...
    # Histograma promedio de todas las alturas
    plt.figure(figsize=(10, 8))
    
    # Combinar todas las alturas
    todas_alturas = np.concatenate([d['height'].flatten() for d in todos_datos])
    
    plt.hist(todas_alturas, bins=50, alpha=0.7, edgecolor='black', density=True)
    plt.title(f'Distribución de Alturas Promedio\n{len(todos_datos)} simulaciones', fontsize=16)
    plt.xlabel('Altura (capas atómicas)', fontsize=14)
    plt.ylabel('Densidad de probabilidad', fontsize=14)
    plt.grid(True, alpha=0.3)
    
    # Calcular y mostrar estadísticas
    media = np.mean(todas_alturas)
    desviacion = np.std(todas_alturas)
    textstr = f'Media: {media:.2f}\nDesviación: {desviacion:.2f}'
    props = dict(boxstyle='round', facecolor='wheat', alpha=0.7)
    plt.text(0.75, 0.95, textstr, transform=plt.gca().transAxes, fontsize=12,
            verticalalignment='top', bbox=props)
    
    plt.savefig(os.path.join(output_dir, 'histograma_promedio.png'), dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    # Procesar cada archivo (solo cargar datos, sin generar gráficas individuales)
    todos_datos = []
