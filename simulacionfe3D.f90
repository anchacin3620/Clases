program MonteCarloSputtering3D_Fe
    ...
    ! Probabilidad de difusión para Fe (Arrhenius)
    ...
    open(unit=11, file='Fe_3d.vtk', status='replace', iostat=iostat)
    if (iostat /= 0) then
        print *, 'Error al abrir el archivo Fe_3d.vtk'
        stop
    end if

    ! Simulación Monte Carlo Cinético 3D para Fe
    do k = 1, nparticles
    ...
    ! Escribir perfil de altura proyectado (Fe_height.dat)
    ...
    ! Escribir datos 3D en formato VTK simplificado (Fe_3d.vtk)
    write(11, '(A)') '# vtk DataFile Version 3.0'
    write(11, '(A)') 'Sputtering 3D Fe on Mg(001) 300x300'
    write(11, '(A)') 'ASCII'
    ...
    print *, 'Simulación completada. Datos en Fe_height.dat y Fe_3d.vtk'
...
contains
    ...
    subroutine interact(xpos, ypos, energy, theta, substrate, chemisorbed, height, nx, ny, Lz)
        ...
        ! Calcular probabilidades basadas en energía, ángulo y propiedades del Fe
        ...
        sputter_prob = 0.05 * (energy / (E_sput_Fe * eV_to_J))  ! Depende de la energía
        ...
        if (r < adsorption_prob) then
            ! Adsorción: depositar Fe en el punto más alto
            ...
            if (zpos <= Lz) then
                substrate(xpos, ypos, zpos) = 2  ! Fe = 2
            ...
            ! Quimisorción: depositar Fe y marcar como quimisorbido
            zpos = height(xpos, ypos) + 1
            do i = max(1, xpos-1), min(nx, xpos+1)
                do j = max(1, ypos-1), min(ny, ypos+1)
                    if (height(i, j) >= zpos) then
